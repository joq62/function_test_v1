...
%    ?debugMsg("Start common"),
%    ?assertEqual(ok,common()),
%    ?debugMsg("stop common"),
...
single_node(Dbase0)->
    ...
    ?assertMatch(["control_100_c1.app_spec","dbase_100_c2.app_spec",
		  "calc_c_100.app_spec","control_100_c0.app_spec",
		  "control_100_c2.app_spec","dbase_100_c0.app_spec",
		  "dbase_100_c1.app_spec","syslog_100_c1.app_spec",
		  "calc_a_100.app_spec","syslog_100_c2.app_spec",
		  "syslog_100_c0.app_spec","calc_b_100.app_spec"],
		 AppSpecsId),
...
setup()->
     %starta vm_nodes
    {ok,ControlVmIds}=application:get_env(control_vm_id),
%    io:format("ControlVmIds ~p~n",[ControlVmIds]),
    {ok,HostId}=net:gethostname(),
 %   ControlNodes=[{list_to_atom(ControlVmId++"@"++HostId)||ControlVmId<-ControlVmIds],
    SlaveStart=[slave:start(HostId,ControlVmId,"-pa "++ControlVmId++" -setcookie abc")||ControlVmId<-ControlVmIds],

    ?assertMatch([{ok,c0@c2},{ok,c1@c2},{ok,c2@c2}],SlaveStart),
   % io:format("SlaveStart ~p~n",[SlaveStart]),
    ok.
%% --------------------------------------------------------------------
%% Function:start/0 
%% Description: Initiate the eunit tests, set upp needed processes etc
